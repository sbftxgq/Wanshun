...
/**
 * 使用装饰模式设计此类，主要用于分页处理场合，不分页时，当做ResultSet来使用，和ResultSet功能一样
 * 只是在ResultSet接口上扩展了几个用于分页的功能。
 * IPageableResultSet接口继承自ResultSet，在使用方法上与ResultSet一致，
 * 尤其是在不需要分页功能的时候，本类可以直接当成ResultSet使用。
 * 
 * @author 李毓峰
 * 
 */
public class IPageableResultSetImpl implements IPageableResultSet {

	protected java.sql.ResultSet rs = null;
	protected int rowsCount;//总记录数
	protected int pageSize;//一页显示的记录数
	 ...
	 * 
	 */
	public IPageableResultSetImpl(java.sql.ResultSet rs)
			throws SQLException {
		if (rs == null)
	...
	public void gotoPage(int page) {
		...
		if (page > getPageCount())
			page = getPageCount();
		int row = (page - 1) * pageSize + 1;//??row代表行号，MySQL中的limit函数起始偏移量为0
		try {
			rs.absolute(row);
			curPage = page;
